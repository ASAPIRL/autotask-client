 ...
 */
class DocumentTicketAssociationQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("DocumentTicketAssociations/query/count", $this->toArray());
        }else{
            $response = $this->client->get("DocumentTicketAssociations/query/count", [
     ...
     * Runs the query.
     */
    public function get(): DocumentTicketAssociationCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("DocumentTicketAssociations/query", $this->toArray());
        }else{
            $response = $this->client->get("DocumentTicketAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return DocumentTicketAssociationCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): DocumentTicketAssociationPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("DocumentTicketAssociations/query", $this->toArray());
            return new DocumentTicketAssociationPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("DocumentTicketAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new DocumentTicketAssociationPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('DocumentTicketAssociations/query')
        );

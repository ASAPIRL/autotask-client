 ...
 */
class ConfigurationItemAttachmentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemAttachments/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ConfigurationItemAttachmentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemAttachments/query", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ConfigurationItemAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ConfigurationItemAttachmentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemAttachments/query", $this->toArray());
            return new ConfigurationItemAttachmentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ConfigurationItemAttachmentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ConfigurationItemAttachments/query')
        );

 ...
 */
class DocumentPlainTextContentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("DocumentPlainTextContent/query/count", $this->toArray());
        }else{
            $response = $this->client->get("DocumentPlainTextContent/query/count", [
     ...
     * Runs the query.
     */
    public function get(): DocumentPlainTextContentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("DocumentPlainTextContent/query", $this->toArray());
        }else{
            $response = $this->client->get("DocumentPlainTextContent/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return DocumentPlainTextContentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): DocumentPlainTextContentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("DocumentPlainTextContent/query", $this->toArray());
            return new DocumentPlainTextContentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("DocumentPlainTextContent/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new DocumentPlainTextContentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('DocumentPlainTextContent/query')
        );

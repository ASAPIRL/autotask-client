...
class ProductTierQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ProductTiers/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ProductTiers/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ProductTierCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ProductTiers/query", $this->toArray());
        }else{
            $response = $this->client->get("ProductTiers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ProductTierCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ProductTierPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ProductTiers/query", $this->toArray());
            return new ProductTierPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ProductTiers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ProductTierPaginator($this->client, $response);
        }
    }

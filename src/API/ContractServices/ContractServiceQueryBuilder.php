 ...
 */
class ContractServiceQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractServices/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ContractServices/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ContractServiceCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractServices/query", $this->toArray());
        }else{
            $response = $this->client->get("ContractServices/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ContractServiceCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ContractServicePaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractServices/query", $this->toArray());
            return new ContractServicePaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ContractServices/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ContractServicePaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ContractServices/query')
        );

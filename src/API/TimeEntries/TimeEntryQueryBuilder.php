...
class TimeEntryQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TimeEntries/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TimeEntries/query/count", [
    ...
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TimeEntries/query", $this->toArray());
        }else{
            $response = $this->client->get("TimeEntries/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TimeEntryCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TimeEntryPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TimeEntries/query", $this->toArray());
            return new TimeEntryPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TimeEntries/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TimeEntryPaginator($this->client, $response);
        }
    }

 ...
 */
class ResourceRoleQueueQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ResourceRoleQueues/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ResourceRoleQueues/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ResourceRoleQueueCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ResourceRoleQueues/query", $this->toArray());
        }else{
            $response = $this->client->get("ResourceRoleQueues/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ResourceRoleQueueCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ResourceRoleQueuePaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ResourceRoleQueues/query", $this->toArray());
            return new ResourceRoleQueuePaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ResourceRoleQueues/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ResourceRoleQueuePaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ResourceRoleQueues/query')
        );

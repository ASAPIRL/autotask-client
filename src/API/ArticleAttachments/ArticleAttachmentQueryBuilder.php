 ...
 */
class ArticleAttachmentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleAttachments/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ArticleAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ArticleAttachmentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleAttachments/query", $this->toArray());
        }else{
            $response = $this->client->get("ArticleAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ArticleAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ArticleAttachmentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleAttachments/query", $this->toArray());
            return new ArticleAttachmentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ArticleAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ArticleAttachmentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ArticleAttachments/query')
        );

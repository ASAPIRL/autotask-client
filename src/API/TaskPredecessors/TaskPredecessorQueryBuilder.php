 ...
 */
class TaskPredecessorQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TaskPredecessors/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TaskPredecessors/query/count", [
     ...
     * Runs the query.
     */
    public function get(): TaskPredecessorCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TaskPredecessors/query", $this->toArray());
        }else{
            $response = $this->client->get("TaskPredecessors/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TaskPredecessorCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TaskPredecessorPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TaskPredecessors/query", $this->toArray());
            return new TaskPredecessorPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TaskPredecessors/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TaskPredecessorPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('TaskPredecessors/query')
        );

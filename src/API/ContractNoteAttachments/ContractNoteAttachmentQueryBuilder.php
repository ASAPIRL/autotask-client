 ...
 */
class ContractNoteAttachmentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractNoteAttachments/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ContractNoteAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ContractNoteAttachmentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractNoteAttachments/query", $this->toArray());
        }else{
            $response = $this->client->get("ContractNoteAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ContractNoteAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ContractNoteAttachmentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractNoteAttachments/query", $this->toArray());
            return new ContractNoteAttachmentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ContractNoteAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ContractNoteAttachmentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ContractNoteAttachments/query')
        );

 ...
 */
class TicketNoteAttachmentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketNoteAttachments/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TicketNoteAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): TicketNoteAttachmentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketNoteAttachments/query", $this->toArray());
        }else{
            $response = $this->client->get("TicketNoteAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TicketNoteAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TicketNoteAttachmentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketNoteAttachments/query", $this->toArray());
            return new TicketNoteAttachmentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TicketNoteAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TicketNoteAttachmentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('TicketNoteAttachments/query')
        );

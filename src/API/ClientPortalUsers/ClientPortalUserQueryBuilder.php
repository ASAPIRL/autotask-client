 ...
 */
class ClientPortalUserQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ClientPortalUsers/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ClientPortalUsers/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ClientPortalUserCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ClientPortalUsers/query", $this->toArray());
        }else{
            $response = $this->client->get("ClientPortalUsers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ClientPortalUserCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ClientPortalUserPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ClientPortalUsers/query", $this->toArray());
            return new ClientPortalUserPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ClientPortalUsers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ClientPortalUserPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ClientPortalUsers/query')
        );

 ...
 */
class TaskNoteAttachmentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TaskNoteAttachments/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TaskNoteAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): TaskNoteAttachmentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TaskNoteAttachments/query", $this->toArray());
        }else{
            $response = $this->client->get("TaskNoteAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TaskNoteAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TaskNoteAttachmentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TaskNoteAttachments/query", $this->toArray());
            return new TaskNoteAttachmentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TaskNoteAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TaskNoteAttachmentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('TaskNoteAttachments/query')
        );

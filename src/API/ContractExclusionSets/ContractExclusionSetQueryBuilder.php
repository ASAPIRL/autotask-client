 ...
 */
class ContractExclusionSetQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractExclusionSets/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ContractExclusionSets/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ContractExclusionSetCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractExclusionSets/query", $this->toArray());
        }else{
            $response = $this->client->get("ContractExclusionSets/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ContractExclusionSetCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ContractExclusionSetPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractExclusionSets/query", $this->toArray());
            return new ContractExclusionSetPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ContractExclusionSets/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ContractExclusionSetPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ContractExclusionSets/query')
        );

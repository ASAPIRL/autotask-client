 ...
 */
class TicketRmaCreditQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketRmaCredits/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TicketRmaCredits/query/count", [
     ...
     * Runs the query.
     */
    public function get(): TicketRmaCreditCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketRmaCredits/query", $this->toArray());
        }else{
            $response = $this->client->get("TicketRmaCredits/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TicketRmaCreditCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TicketRmaCreditPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketRmaCredits/query", $this->toArray());
            return new TicketRmaCreditPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TicketRmaCredits/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TicketRmaCreditPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('TicketRmaCredits/query')
        );

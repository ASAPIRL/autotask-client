...
class RoleQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("Roles/query/count", $this->toArray());
        }else{
            $response = $this->client->get("Roles/query/count", [
    ...
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("Roles/query", $this->toArray());
        }else{
            $response = $this->client->get("Roles/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return RoleCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): RolePaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("Roles/query", $this->toArray());
            return new RolePaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("Roles/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new RolePaginator($this->client, $response);
        }
    }

 ...
 */
class OrganizationalLevelAssociationQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("OrganizationalLevelAssociations/query/count", $this->toArray());
        }else{
            $response = $this->client->get("OrganizationalLevelAssociations/query/count", [
     ...
     * Runs the query.
     */
    public function get(): OrganizationalLevelAssociationCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("OrganizationalLevelAssociations/query", $this->toArray());
        }else{
            $response = $this->client->get("OrganizationalLevelAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return OrganizationalLevelAssociationCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): OrganizationalLevelAssociationPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("OrganizationalLevelAssociations/query", $this->toArray());
            return new OrganizationalLevelAssociationPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("OrganizationalLevelAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new OrganizationalLevelAssociationPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('OrganizationalLevelAssociations/query')
        );

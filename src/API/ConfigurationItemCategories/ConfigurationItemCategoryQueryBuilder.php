 ...
 */
class ConfigurationItemCategoryQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemCategories/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemCategories/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ConfigurationItemCategoryCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemCategories/query", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemCategories/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ConfigurationItemCategoryCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ConfigurationItemCategoryPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemCategories/query", $this->toArray());
            return new ConfigurationItemCategoryPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemCategories/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ConfigurationItemCategoryPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ConfigurationItemCategories/query')
        );

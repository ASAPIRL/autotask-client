 ...
 */
class ConfigurationItemQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItems/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItems/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ConfigurationItemCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItems/query", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItems/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ConfigurationItemCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ConfigurationItemPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItems/query", $this->toArray());
            return new ConfigurationItemPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItems/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ConfigurationItemPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ConfigurationItems/query')
        );

 ...
 */
class ContactWebhookQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContactWebhooks/query/count", $this->toArray());
        }else{
     ...
     * Runs the query.
     */
    public function get(): ContactWebhookCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContactWebhooks/query", $this->toArray());
        }else{
            $response = $this->client->get("ContactWebhooks/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ContactWebhookCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ContactWebhookPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContactWebhooks/query", $this->toArray());
            return new ContactWebhookPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ContactWebhooks/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ContactWebhookPaginator($this->client, $response);
        }
    }

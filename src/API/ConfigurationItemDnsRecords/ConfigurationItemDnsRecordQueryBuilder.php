 ...
 */
class ConfigurationItemDnsRecordQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemDnsRecords/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemDnsRecords/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ConfigurationItemDnsRecordCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemDnsRecords/query", $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemDnsRecords/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ConfigurationItemDnsRecordCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ConfigurationItemDnsRecordPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ConfigurationItemDnsRecords/query", $this->toArray());
            return new ConfigurationItemDnsRecordPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ConfigurationItemDnsRecords/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ConfigurationItemDnsRecordPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ConfigurationItemDnsRecords/query')
        );

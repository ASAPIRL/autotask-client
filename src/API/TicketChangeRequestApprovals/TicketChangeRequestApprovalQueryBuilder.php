 ...
 */
class TicketChangeRequestApprovalQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketChangeRequestApprovals/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TicketChangeRequestApprovals/query/count", [
     ...
     * Runs the query.
     */
    public function get(): TicketChangeRequestApprovalCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketChangeRequestApprovals/query", $this->toArray());
        }else{
            $response = $this->client->get("TicketChangeRequestApprovals/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TicketChangeRequestApprovalCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TicketChangeRequestApprovalPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketChangeRequestApprovals/query", $this->toArray());
            return new TicketChangeRequestApprovalPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TicketChangeRequestApprovals/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TicketChangeRequestApprovalPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('TicketChangeRequestApprovals/query')
        );

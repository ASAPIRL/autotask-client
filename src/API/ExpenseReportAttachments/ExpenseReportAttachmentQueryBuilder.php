 ...
 */
class ExpenseReportAttachmentQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ExpenseReportAttachments/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ExpenseReportAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ExpenseReportAttachmentCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ExpenseReportAttachments/query", $this->toArray());
        }else{
            $response = $this->client->get("ExpenseReportAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ExpenseReportAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ExpenseReportAttachmentPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ExpenseReportAttachments/query", $this->toArray());
            return new ExpenseReportAttachmentPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ExpenseReportAttachments/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ExpenseReportAttachmentPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ExpenseReportAttachments/query')
        );

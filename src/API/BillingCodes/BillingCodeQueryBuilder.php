...
class BillingCodeQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("BillingCodes/query/count", $this->toArray());
        }else{
     ...
     * Runs the query.
     */
    public function get(): BillingCodeCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("BillingCodes/query", $this->toArray());
        }else{
            $response = $this->client->get("BillingCodes/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return BillingCodeCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): BillingCodePaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("BillingCodes/query", $this->toArray());
            return new BillingCodePaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("BillingCodes/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new BillingCodePaginator($this->client, $response);
        }
    }

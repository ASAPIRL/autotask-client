 ...
 */
class ArticleToArticleAssociationQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleToArticleAssociations/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ArticleToArticleAssociations/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ArticleToArticleAssociationCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleToArticleAssociations/query", $this->toArray());
        }else{
            $response = $this->client->get("ArticleToArticleAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ArticleToArticleAssociationCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ArticleToArticleAssociationPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleToArticleAssociations/query", $this->toArray());
            return new ArticleToArticleAssociationPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ArticleToArticleAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ArticleToArticleAssociationPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ArticleToArticleAssociations/query')
        );

 ...
 */
class BillingItemApprovalLevelQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("BillingItemApprovalLevels/query/count", $this->toArray());
        }else{
            $response = $this->client->get("BillingItemApprovalLevels/query/count", [
     ...
     * Runs the query.
     */
    public function get(): BillingItemApprovalLevelCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("BillingItemApprovalLevels/query", $this->toArray());
        }else{
            $response = $this->client->get("BillingItemApprovalLevels/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return BillingItemApprovalLevelCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): BillingItemApprovalLevelPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("BillingItemApprovalLevels/query", $this->toArray());
            return new BillingItemApprovalLevelPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("BillingItemApprovalLevels/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new BillingItemApprovalLevelPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('BillingItemApprovalLevels/query')
        );

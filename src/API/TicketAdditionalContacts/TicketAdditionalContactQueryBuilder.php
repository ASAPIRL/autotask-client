 ...
 */
class TicketAdditionalContactQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketAdditionalContacts/query/count", $this->toArray());
        }else{
            $response = $this->client->get("TicketAdditionalContacts/query/count", [
     ...
     * Runs the query.
     */
    public function get(): TicketAdditionalContactCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketAdditionalContacts/query", $this->toArray());
        }else{
            $response = $this->client->get("TicketAdditionalContacts/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return TicketAdditionalContactCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): TicketAdditionalContactPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("TicketAdditionalContacts/query", $this->toArray());
            return new TicketAdditionalContactPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("TicketAdditionalContacts/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new TicketAdditionalContactPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('TicketAdditionalContacts/query')
        );

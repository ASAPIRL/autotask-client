...
class ContactQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("Contacts/query/count", $this->toArray());
        }else{
            $response = $this->client->get("Contacts/query/count", [
    ...
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("Contacts/query", $this->toArray());
        }else{
     ...
     */
    public function paginate(): ContactPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("Contacts/query", $this->toArray());
            return new ContactPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("Contacts/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ContactPaginator($this->client, $response);
        }
    }

 ...
 */
class ContractBlockHourFactorQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractBlockHourFactors/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ContractBlockHourFactors/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ContractBlockHourFactorCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractBlockHourFactors/query", $this->toArray());
        }else{
            $response = $this->client->get("ContractBlockHourFactors/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ContractBlockHourFactorCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ContractBlockHourFactorPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ContractBlockHourFactors/query", $this->toArray());
            return new ContractBlockHourFactorPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ContractBlockHourFactors/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ContractBlockHourFactorPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ContractBlockHourFactors/query')
        );

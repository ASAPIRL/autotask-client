 ...
 */
class ArticleTagAssociationQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleTagAssociations/query/count", $this->toArray());
        }else{
            $response = $this->client->get("ArticleTagAssociations/query/count", [
     ...
     * Runs the query.
     */
    public function get(): ArticleTagAssociationCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleTagAssociations/query", $this->toArray());
        }else{
            $response = $this->client->get("ArticleTagAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return ArticleTagAssociationCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): ArticleTagAssociationPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("ArticleTagAssociations/query", $this->toArray());
            return new ArticleTagAssociationPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("ArticleTagAssociations/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new ArticleTagAssociationPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('ArticleTagAssociations/query')
        );

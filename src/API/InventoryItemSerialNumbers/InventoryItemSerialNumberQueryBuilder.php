 ...
 */
class InventoryItemSerialNumberQueryBuilder
{
     ...
     */
     public function count(): int
     {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("InventoryItemSerialNumbers/query/count", $this->toArray());
        }else{
            $response = $this->client->get("InventoryItemSerialNumbers/query/count", [
     ...
     * Runs the query.
     */
    public function get(): InventoryItemSerialNumberCollection
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("InventoryItemSerialNumbers/query", $this->toArray());
        }else{
            $response = $this->client->get("InventoryItemSerialNumbers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
        }

        return InventoryItemSerialNumberCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): InventoryItemSerialNumberPaginator
    {
        if (strlen($this->__toString()) >= self::GET_LIMIT) {
            $response = $this->client->post("InventoryItemSerialNumbers/query", $this->toArray());
            return new InventoryItemSerialNumberPaginator($this->client, $response, $this->toArray());
        }else{
            $response = $this->client->get("InventoryItemSerialNumbers/query", [
                'search' => json_encode( $this->toArray() )
            ]);
            return new InventoryItemSerialNumberPaginator($this->client, $response);
        }
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('InventoryItemSerialNumbers/query')
        );
